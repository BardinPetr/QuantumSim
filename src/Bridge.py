...
from src.math.algebra import ip_str_to_bytes, ip_bytes_to_str
...
class Bridge(Eventable):
    ...
    def _process_incoming_packets(self):
        ...
        if header == Bridge.HEADER_CTRL:
            ...
            elif header == Bridge.HEADER_CLASSIC:
                self.emit(Bridge.EVENT_SOCKET_INCOMING, (from_ip, data))
    ...
    def _process_outgoing_packets(self, idx=0):
        ...
        for target_ip in ([msg_target_ip] if msg_target_ip != Bridge.BROADCAST_IP else self.connections.keys()):
            ...
            try:
                L.debug(f"SENDING TO {target_ip} VIA {ip} MSG data: {data[:25]}")
    ...
    def run(self):
        ...
        [i.start() for i in self.threads[-2:]]
